...
import 'package:habits_tracker/ui/widgets/date_picker/customization/calendar_builders.dart';
...
import 'package:habits_tracker/ui/widgets/date_picker/widgets/cell_widget.dart';
import 'package:habits_tracker/ui/widgets/date_picker/widgets/custom_icon_button.dart';
...
class _TableCalendarState extends State<TableCalendar> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(TableCalendar oldWidget) {
    ...
    if (oldWidget.availableCalendarFormats != widget.availableCalendarFormats) {
      widget.calendarController.availableCalendarFormats = widget.availableCalendarFormats;
    }
  }
  ...
  bool _isDayUnavailable(DateTime day) {
    return (widget.startDay != null && day.isBefore(widget.calendarController.normalizeDate(widget.startDay))) ||
        (widget.endDay != null && day.isAfter(widget.calendarController.normalizeDate(widget.endDay))) ||
        (!_isDayEnabled(day));
  }
  ...
  Widget _buildTable() {
    ...
    int x = 0;
    while (x < widget.calendarController.visibleDay.value.length) {
      children.add(_buildTableRow(widget.calendarController.visibleDay.value.skip(x).take(daysInWeek).toList()));
      x += daysInWeek;
    }
  ...
  TableRow _buildDaysOfWeek() {
    return TableRow(
      ...
      children: widget.calendarController.visibleDay.value.take(7).map((date) {
        ...
        final isWeekend = widget.calendarController.isWeekend(date, widget.weekendDays);
  ...
  Widget _buildCellContent(DateTime date) {
    ...
    final tIsWeekend = widget.calendarController.isWeekend(date, widget.weekendDays);
